...
\item \textbf{Postural baseline (E1).}
  ...
  per posture, this duration is fundamentally inadequate for
...
\item \textbf{Voluntary breath-hold (E2).}
  ...
  thereby improving the statistical stability of hold-phase
  time-domain metrics (HR slope, RMSSD) and increasing the spectral
...
\begin{itemize}
  ...
  \item \textbf{RMSSD}: the root mean square of successive NN
    ...
    boundary definitions and therefore provides a robust vagal proxy
    even when breathing frequency migrates across the LF/HF boundary.
...
\bottomrule
\multicolumn{6}{@{}l}{\footnotesize $^\ast$SDNN is
  duration-dependent~\cite{taskforce1996}; condition durations ranged
  from 220 to 280~s.}
\end{tabular}
\end{table}
...
\caption{Dual-path validation summary. (a)~Per-session artifact rate.
(b)~Bland--Altman comparison of mean NN intervals between the SciPy
teaching path and the NeuroKit2 reference path. (c)~Bland--Altman
comparison of RMSSD between the two paths.}
...
fell from 26.4\% at 50~s to 5.8\% at 250~s. HF was even more robust,
dropping to 3.6\% CV by 250~s. LF remained the noisiest band
(CV $>$ 27\% even at 250~s), reflecting its sensitivity to slow trends
and sub-band drift. These results justify our use of full multi-minute
...
the E1A supine recording. SDNN and HF stabilize substantially by
$\sim$180--250~s, whereas LF remains comparatively variable.}
...
trials to highlight the effort/relaxation contrast central to the H2
argument. This appendix provides the complementary
inspiratory-vs.-expiratory overlay restricted to the relaxed trial,
thereby isolating the pure respiratory-cessation effect without the
additional voluntary-strain confound present in Trial~1.
...
inspiratory condition is consistent with elevated intrathoracic
...
descent from its higher peak. This reinforces the interpretation from
...
\caption{Inspiratory vs.\ expiratory relaxed-trial breath-hold overlay,
time-aligned to hold onset. The inspiratory hold produces the earlier
and larger HR excursion, isolating the physiological effect of lung
inflation apart from voluntary effort.}
...
cadence-coincident ECG spectral contamination. The figures below add
...
reliable R-peak detection. This verifies that the motion-limited HRV
interpretation in Section~\ref{ssec:walking} reflects genuine
physiological measurement uncertainty under ambulation, not detector
failure.
...
consistent with autonomic ``locked-in'' behavior under elevated
sympathetic drive combined with motion-induced spectral spreading. In
that sense, the figure complements Fig.~\ref{fig:walk_ecg_psd}: the
main-text panel shows where locomotor contamination enters the ECG
spectrum, while the appendix contrast shows the resulting flattening
of physiologically interpretable RR structure.

Taken together, these two panels support a bounded conclusion for E3:
walking still permits robust extraction of average rate and recovery
...
\caption{Walking vs.\ seated-baseline RR power spectral density
contrast. Walking shows near-collapse of HF power together with
broadening of the LF region, consistent with sympathetic activation
plus motion-related spectral spreading.}
...
qualitative shape---and critically, the absence of an inverted-U---is
preserved.

This robustness check supports the interpretation that the negative
result is a genuine property of this participant's response under the
fixed-order protocol, not a methodological artifact. It therefore
strengthens the main-text argument that the paced-breathing
...
All three bandwidths show monotonic increase from 12 to 3~breaths/min
with no isolated peak at 6~breaths/min (dashed vertical line).}
