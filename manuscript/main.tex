...
This single-participant lab study analyzed 15~single-lead ECG recordings
...
($R^{2} = 0.998$); LF/HF inflated from 0.22 to 17.47 without parallel
RMSSD collapse, indicating spectral-band migration rather than
...
spectral band migration, biomedical sensor lab
\end{IEEEkeywords}
...
We acquired 15~single-lead ECG recordings (500~Hz) from one participant
using a unified analysis pipeline spanning (1)~postural baseline,
(2)~voluntary breath-hold, (3)~ambulatory walking with recovery,
(4)~paced breathing at five rates, and (5)~a partial sleep-onset segment
...
Q4: Self-designed variation & E4A: paced breathing & \ref{ssec:paced} &
...
All 15~recordings were acquired in a single session on April~24,
...
The protocol comprised five experiment families
...
\item \textbf{Paced breathing (E4A).}
...
\item \textbf{Sleep onset (E4B).}
  A 14.3-min partial sleep-onset recording was obtained for
  exploratory purposes only and is reported in the Appendix.
\end{enumerate}
...
\midrule
E4A Paced & 12, 9, 6, 5, 3/min & 220--280 & Sitting \\
\midrule
E4B Sleep & Partial recording & 860 & Supine \\
\bottomrule
\end{tabular}
\end{table}
...
rejected.
For steady-state conditions (E1, E4A), the full analysis window
($\geq$220~s) was used.
...
% source file to nb02_fig05_e1_poincare_plots.pdf (no accent).
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{nb02_fig05_e1_Poincaré_plots.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{nb03_fig02_e2_effort_comparison.pdf}
...
Magnitude-squared coherence between the ECG and accelerometer
signals was low both at the step peak ($C_{xy} = 0.04$) and across
the 1--3~Hz cadence band (mean $C_{xy} = 0.06$, max~0.20);
Pearson correlation between $|\Delta\text{RR}|$ and accelerometer
magnitude was likewise weak ($r = 0.26$).
These low values are \emph{expected} rather than exculpatory:
the dominant artifact pathways (contact impedance changes,
baseline wander) are broadband and stochastic,
and therefore couple poorly with the quasi-periodic step signal
measured by the accelerometer.
Low linear coherence thus does not imply artifact-free RR intervals;
it indicates that the contamination is non-linear and cannot be
removed by coherence-guided spectral subtraction.
...
with near-perfect fidelity (Fig.~\ref{fig:psd_overlay}, inset).
...
RMSSD (ms)         & 47.7  & 78.0  & 60.4  & 53.9  & 55.3 \\
...
\addlinespace
Paced $\geq$9/min (E4A) & Mostly valid & RMSSD, HF, LF/HF & Resp.\ peak in or near HF band \\
\addlinespace
Paced $\leq$6/min (E4A) & Band-shifted & RMSSD, total power, RR ampl., resp.-centered power & LF/HF invalid as sympathovagal index; use resp.-centered power for resonance \\
\bottomrule
\end{tabular}
\end{table}
...
was low ($C_{xy} = 0.04$ at the step peak), suggesting that motion
contamination was intermittent rather than phase-locked; nonetheless,
the combination of high accelerometer motion and cadence-proximate
spectral energy warrants caution for fine-scale HRV indices.

Fifth, the sleep-onset recording (E4B) was incomplete (14.3~min) and
supports only appendix-level exploratory trend reporting.
...
\emph{J.\ Physiol.}, vol.~548, no.~2, pp.~339--352, 2003.
...
Across all 9~steady-state sessions, peak-count agreement
exceeded 99.5\%.
...
\section{Exploratory Sleep-Onset Recording (E4B)}
\label{app:sleep}

A 14.3-min partial sleep-onset recording showed a smoothed HR
decrease of approximately 2.6~bpm (from 55.3 to 52.7~bpm),
approaching the E1A supine anchor of 54.0~bpm.
This directionally plausible trend is consistent with early
parasympathetic dominance during sleep onset, but the recording
was too short and incomplete for formal HRV analysis.
The trajectory is shown in Fig.~\ref{fig:sleep}.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{nb06_fig04_e4b_sleep_trajectory.pdf}
\caption{Partial sleep-onset HR trajectory.  The smoothed HR trace
shows a gradual decline toward the supine baseline, consistent with
increasing vagal tone during sleep onset.  This recording is
exploratory and should not be interpreted as steady-state HRV.}
\label{fig:sleep}
\end{figure}
